...
<div id="central">
  ...
  <div id="apDiv9">
    ...
    <table width="487" border="1" cellspacing="0" cellpadding="0">
	  ...
	  $qry=mysql_query("select * from spots");
	  $num=mysql_num_rows($qry);
	  if($num==0)
	  {
		  header("Location:spotsempty.php");
	  }
	  while($row=mysql_fetch_array($qry))
	  {
		  print "<tr>";
		  print "<form name='f1' method='POST' action='update_dest1.php'>";
		  print "<td align='center' >";
		  print "<input type='text' class='txt' name='spotname' value='".$row['Spot_name']."'>";
		   print "<input type='hidden' name='h' value='".$row['Spot_name']."'>";
		  print "</td>";
		  print "<td align='center' >";
		   print "<input type='text' class='txt'  name='detail' value='".$row['Details']."'>";
		  print "</td>";
		  print "<td align='center'>";
		   $pid=$row['pid'];
		   print"<select name='pid'>";
		   $qry1=mysql_query("select * from package where pid='$pid'");
...
include("connection.php");
  extract($_POST);
  $qry=mysql_query("update spots set Spot_name='$spotname',Details='$detail',pid='$pid' where Spot_name='$h'");
  if($qry)
  {
  ...
  ?>
  <div id="apDiv10"><a href="update_spots.php">x</a></div>
</div>
</body>
</html>